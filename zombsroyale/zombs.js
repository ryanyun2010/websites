...
var gottonshield=false;
var canvas = document.querySelector('canvas');
var tools = canvas.getContext('2d');
var tile='https://ryan1.wisen.space/capture/load/uNqbBknOrI5cDJlxfTJI3tze.png'
...
var shield =new Weapon({width:60,height:60,image:"https://ryan1.wisen.space/capture/load/vzbmnkkoxgmtui2uftpvsr42.jpg"})
...
var shielditem =new INVENTORYITEM("https://ryan1.wisen.space/capture/load/vzbmnkkoxgmtui2uftpvsr42.jpg",3,'shield')
...
resources.push(inventory,bambooitem,gunitem,shield,shielditem);
//test=1;
let id = function (id) {
...
function Bar(){
    this.draw=function(){
        ...
        id("bar").style.background="green";
	id("bar").style.position="absolute";
	id("bar").style.top=canvas.height-40+"px";
			id("bar").style.left="40px";
			id("bar").style.width=health.number*2+"px";
			id("bar").style.height="40px";
    }
    this.move=function(){
    ...
    this.draw=function(){
        if(this.item==='bamboo'){
         ...
         if(gottonweaponbamboogun){
			...
			console.log("大熊猫")
			if(gottonshield){
				console.log("Dà xióngmāo")
				tools.drawImage(this.image, (canvas.width/2)+510,canvas.height-30, 30, 30);
				 }
				 }
	}
	this.move=function(){}
...
function INVENTORYBAR(){
    this.draw=function(){
        ...
        tools.fillText("3",canvas.width/2+500,canvas.height-20);
...
function Score(x){
   ...
   tools.font = "30px Arial";
    ...
    console.log(x)
	this.draw=function(){
    	id("health").style.position="absolute";
	id("health").innerHTML=this.number;
		id("health").style.top=canvas.height-30+"px";
		id("health").style.left="50px";
